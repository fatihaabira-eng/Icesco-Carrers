...
interface CreateJobOfferFormProps {
  onClose?: () => void;
  onCreate?: (newOffer: any) => void;
  onEdit?: (updatedOffer: any) => void;
  jobOffer?: any;
  mode?: 'create' | 'edit';
}
...
const CreateJobOfferForm: React.FC<CreateJobOfferFormProps> = ({ onClose, onCreate, onEdit, jobOffer, mode = 'create' }) => {
  ...
  const [formData, setFormData] = useState({
    ...
    contractType: jobOffer?.contractType || 'full-time',
    tasks: jobOffer?.tasks || [],
    academicQualifications: jobOffer?.academicQualifications || '',
    preferredCertifications: jobOffer?.preferredCertifications || '',
    salaryBenefits: jobOffer?.salaryBenefits || '',
    closingDate: jobOffer?.closingDate ? new Date(jobOffer.closingDate) : undefined,
    publishedDate: jobOffer?.publishedDate ? new Date(jobOffer.publishedDate) : undefined,
    createdDate: jobOffer?.createdDate ? new Date(jobOffer.createdDate) : new Date(),
    expectedJoiningDate: jobOffer?.expectedJoiningDate ? new Date(jobOffer.expectedJoiningDate) : undefined,
    location: jobOffer?.location || '',
    status: jobOffer?.status || 'draft',
    priority: jobOffer?.priority || 'active',
    positionsNeeded: jobOffer?.positionsNeeded || 1,
    businessUnit: jobOffer?.businessUnit || '',
  });
  ...
  const handleSubmit = () => {
    ...
    const jobOfferData = {
      ...formData,
      skills,
      id: mode === 'edit' && jobOffer?.id ? jobOffer.id : `JOB-${Date.now()}`,
      ...
      applicants: jobOffer?.applicants || 0,
      shortlisted: jobOffer?.shortlisted || 0,
    };

    if (mode === 'edit' && onEdit) {
      toast({
        title: 'Job Offer Updated',
        description: `Job posting for ${formData.jobTitle} has been updated.`,
      });
      onEdit(jobOfferData);
    } else {
      toast({
        ...
        <CardHeader>
          <div className="flex items-center space-x-3">
            <div className="flex items-center justify-center w-12 h-12 bg-primary/10 rounded-lg">
              {mode === 'edit' ? <Save className="h-6 w-6 text-primary" /> : <PlusCircle className="h-6 w-6 text-primary" />}
            </div>
            <div>
              <CardTitle className="text-xl">{mode === 'edit' ? 'Edit Job Offer' : 'Create New Job Offer'}</CardTitle>
              <p className="text-muted-foreground">
                {mode === 'edit' ? 'Update the job posting details.' : 'Post a new position to attract qualified candidates'}
              </p>
            </div>
          </div>
        </CardHeader>
        ...
        <CardContent className="pt-6">
          <div className="flex space-x-3">
            <Button onClick={handleSubmit} className="flex items-center space-x-2">
              ...
              <span>{mode === 'edit' ? 'Save Changes' : 'Create Job Offer'}</span>
            </Button>
            {onClose && (
